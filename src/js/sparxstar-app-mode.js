...
const SparxstarApp = {
    ...
    init() {
        if (window.__sparxstarAppInitialized) return;
        ...
        window.addEventListener("popstate", () => {
            if (this.activeElement) this._cleanupUI();
        });
    ...
    /* === OPEN LIFECYCLE === */
    open(element) {
        if (window.innerWidth > this.threshold) return;
        if (this.activeElement) return;
        ...
        requestAnimationFrame(() => {
            if (this.activeElement) element.focus({ preventScroll: true });
        });
    ...
    _onTouchStart(e) {
        ...
        if (this.activeElement.scrollTop > 0) return;
    ...
    _onTouchEnd(e) {
        ...
        if (delta > 100 && velocity > 0.3 && this.activeElement.scrollTop <= 0) {
            ...
            el.style.transform = `scale(${this.currentScale})`;
            setTimeout(() => {
                if (this.activeElement === el) el.style.transition = "";
            }, 300);
        }
    }
};
